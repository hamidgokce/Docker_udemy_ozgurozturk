...
olarak yaratın. Host'un 8080 tcp portuna gelen isteklerin bu containerın 80 portuna 
gitmesini sağlayın.
...
Ardından bu container'ın loglarına erişerek az önceki erişimlerinizin loglarını kontrol edin. 

browserdan 127.0.0.1:8080 ni acalim 
docker logs ...

5: Container loglarını follow modunda takip ederken browser'dan bu web sitesinde olmayan 
bir url'e gitmeye çalışın. Örneğin xyz.html Bunun ürettiği hatayı canlı olarak loglardan
takip edin. 

docker ps
docker logs ... -f (-f canli gormemizi saglamaktadir)
...
-dit ile yaratın sh shellini açın. Bu container default bridge network'e bağlı olsun. 
...
8: Container'a attach işlemiyle bağlanın ve container içerisinden web1'i pinglemeye çalışın. 
Container'ı kapamadan çıkın. 
...
14: Browser'dan websrv container'ının yayınladığı web sitesine bağlanın. 
